...
- **Users API** – `POST /api/users` accepts JSON `{ "email": "<string>", "password": "<string>" }`, hashes password, creates user; returns 201 with id, email, timestamps (no password); invalid/missing email or password → 400, duplicate email → 409. `POST /api/login` accepts `{ "email", "password", "expiresInSeconds"?: number }`, authenticates; returns 200 with user resource plus `token` (JWT); optional `expiresInSeconds` defaults to 1 hour (capped); 401 on failure
...
- **Authentication** – Password hashing (Argon2); login returns JWT; `getBearerToken`, `makeJWT`, `validateJWT`; `JWT_SECRET` in env for signing/verifying
