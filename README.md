...
The server runs migrations at startup, then listens on `PORT`, serves static files at `/app`, exposes readiness at `GET /api/healthz`, metrics at `GET /admin/metrics`, reset at `POST /admin/reset`, users at `POST /api/users`, login at `POST /api/login`, refresh at `POST /api/refresh`, revoke at `POST /api/revoke`, chirps at `GET /api/chirps`, `GET /api/chirps/:chirpId`, and `POST /api/chirps`, and Polka webhooks at `POST /api/polka/webhooks`.
...
_Last updated: March 2025 — Chirpy Red membership; Polka webhooks with API key verification; `POST /api/polka/webhooks`; `isChirpyRed` in user responses._
